...
const normalizeConfidenceFromScore = (score: number): ResearchConfidence => {
...
const normalizeVerdictFromSignals = ({
  ...
  clearancePath: "CLEAR" | "NARROW" | "FRAGILE";
  ...
  if (clearancePath === "FRAGILE") return "LESS";
...
export const normalizeResearchVerdict = (result: ResearchOutput): NormalizedResearchResult => {
  const normalizationNotes: string[] = [];
  const nextResult: ResearchOutput = structuredClone(result);
